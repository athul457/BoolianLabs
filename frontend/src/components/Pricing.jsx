...
const Pricing = () => {
  ...
  return (
    <section
      className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white py-16 md:py-24 transition-colors duration-300"
      id="pricing"
    >
...
const PricingCard = ({ plan }) => (
  <div className="h-full rounded-3xl p-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2 flex flex-col">

    {plan.recommended && (
      <span className="inline-block px-4 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase mb-6">
        Most Popular
      </span>
    )}
    ...
    <div className="flex items-baseline mb-8">
      ...
      {plan.price !== 'Custom' && (
        <span className="ml-2 text-gray-500 dark:text-gray-400">/project</span>
      )}
    </div>
    ...
    <ul className="space-y-4 mb-8 flex-grow">
      {plan.features.map((feature, idx) => (
        <li key={idx} className="flex items-start">
          <Check className="h-4 w-4 text-blue-500 mr-3 mt-1" />
    ...
    <Link
      to="/contact"
      className="mt-auto w-full text-center px-6 py-3 rounded-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:opacity-90 transition-all flex items-center justify-center"
    >
      Choose {plan.title}
